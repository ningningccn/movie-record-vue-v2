            ...
            <div
                ...
                <div>
                    ...
                    <div class="mt-8 space-y-6">
                        ...
                        <Input :placeholder="'原始名稱(選項)'" v-model:text="movieData.original_name" />
                ...
                <button type="button" class="absolute right-10 top-10" @click="vfm.close('add-movie-id')">
...
const handleAddMovie = async () => {
    ...
    if (props.poster_img) movieData.value.postImageUrl = `https://image.tmdb.org/t/p/w780${props.poster_img}`
    else movieData.value.postImageUrl = await uploadMovieImage(tempImage.value)
...
const isSubmit = computed(() => {
    if (movieData.value.country && movieData.value.type && movieData.value.name && movieData.value.year) {
        return true
    } else return false
})
